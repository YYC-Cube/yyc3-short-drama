...
import { Menu, X, User, Settings, LogOut, Crown, Sparkles, type LucideIcon } from "lucide-react"
...
/** 业务页头传入的展示配置（各业务 header 组件的统一 props） */
export interface UniversalHeaderProps {
  title?: string
  subtitle?: string
  icon?: LucideIcon
  backgroundImage?: string
  accentColor?: "amber" | "green" | "purple" | "blue" | "red"
  height?: "sm" | "md" | "lg"
}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
            ...
            <Link href="/main" className="flex items-center gap-3 group">
              ...
              >
                {Icon ? <Icon className="w-6 h-6 text-white" /> : <Crown className="w-6 h-6 text-white" />}
              </motion.div>
            <div className="hidden sm:block">
              <motion.h1
                className="font-bold text-xl bg-linear-to-r from-amber-300 via-orange-300 to-amber-300 bg-clip-text text-transparent group-hover:from-amber-200 group-hover:to-orange-200 transition-all duration-300"
                whileHover={{ scale: 1.05 }}
              >
                {title || "言语逸品"}
              </motion.h1>
              <p className="text-amber-300/80 text-sm font-medium">{subtitle || "河洛文化数字传承平台"}</p>
            </div>
          </Link>
                ...
                <Link key={item.name} href={item.href}>
                  ...
                  >
                    ...
                    {isActive && (
                      <motion.div
                        layoutId="activeTab"
                        className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-linear-to-r from-amber-400 to-orange-500 rounded-full"
                        initial={false}
                    ...
                    {/* 悬浮光效 */}
                    <motion.div
                      className="absolute inset-0 bg-linear-to-r from-transparent via-white/5 to-transparent opacity-0 group-hover:opacity-100 rounded-xl"
                      animate={{
                        x: ["0%", "100%"],
                      }}
                      transition={{
          ...
          <div className="flex items-center gap-3">
            {/* 创作者认证标识 */}
            <motion.div
              className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-linear-to-r from-purple-500/20 to-pink-500/20 rounded-full border border-purple-400/30"
              whileHover={{ scale: 1.05 }}
            >
              ...
              <DropdownMenuTrigger asChild>
                <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="w-10 h-10 rounded-xl bg-linear-to-br from-amber-500/20 to-orange-500/20 border border-amber-500/30 hover:border-amber-500/50 text-amber-300 hover:text-amber-200 transition-all duration-300"
                  >
            ...
            <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
              <Button
                variant="ghost"
                size="icon"
                className="lg:hidden w-10 h-10 rounded-xl bg-linear-to-br from-amber-500/20 to-orange-500/20 border border-amber-500/30 hover:border-amber-500/50 text-amber-300 hover:text-amber-200 transition-all duration-300"
                    ...
                    >
                      ...
                      {isActive && (
                        <motion.div
                          initial={{ scale: 0 }}
                          animate={{ scale: 1 }}
                          className="w-2 h-2 bg-linear-to-r from-amber-400 to-orange-500 rounded-full"
                        />
                      )}
                    </motion.div>
                  </Link>
